...
func (s *Service) GetLangByCode(ctx context.Context, lang string) (string, error) {
	...
	if err != nil {
		return "", msgerror.NewError(fmt.Errorf("language.Service.GetLanguage: %v", err), msgerror.ErrInternal)
	}
...
func (s *Service) GetAvailableLanguages(ctx context.Context) ([]Language, error) {
	...
	if err != nil {
		return nil, msgerror.NewError(fmt.Errorf("language.Service.GetAvailableLanguages: %v", err), msgerror.ErrInternal)
	}
...
func (s *Service) CheckLanguage(ctx context.Context, langCode string) error {
	...
	if _, err := s.repo.GetLanguage(ctx, langCode); err != nil {
		return msgerror.NewError(fmt.Errorf("language.Service.CheckLanguage: %w", err), msgerror.ErrInternal)
	}
	return nil
}
